"""Сид ниши «кондиционерщик»: прайс (13 позиций) и 5 комплектов для пользователя.

Запуск в контейнере:
    docker compose exec api python scripts/seed_hvac.py <user_id>

Повторный запуск для того же пользователя не дублирует данные:
если у него уже есть hvac-позиции, скрипт завершается с ошибкой.
"""
import sys
